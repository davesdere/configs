...
DEBUG=
VERBOSE=
PMC_URL=https://packages.any_company.com/config
SCALED_VERSION=
VERSION=
ONBOARDING_SCRIPT=
MIN_REQUIREMENTS=
...
ERR_ANOTHER_NAME_FOR_SOFTWARE_NOT_INSTALLED=20
...
print_state()
{
    if [ -z $(which this_software_name) ]; then
        echo "[S] ANOTHER_NAME_FOR_SOFTWARE not installed."
    else
        echo "[S] ANOTHER_NAME_FOR_SOFTWARE installed."
...
verify_connectivity()
{
    ...
    if which wget; then
        connect_command="wget -O - --quiet --no-verbose --timeout 2 https://cdn.x.cp.wd.any_company.com/ping --no-check-certificate"
    elif which curl; then
        connect_command="curl --silent --connect-timeout 2 --insecure https://cdn.x.cp.wd.any_company.com/ping"
    else
...
verify_min_requirements()
{
    ...
    if [ $cores -lt $MIN_CORES ]; then
        script_exit "ANOTHER_NAME_FOR_SOFTWARE requires $MIN_CORES cores or more to run, found $cores." $ERR_INSUFFICIENT_REQUIREMENTS
    fi

    local mem_mb=$(free -m | grep Mem | awk '{print $2}')
    if [ $mem_mb -lt $MIN_MEM_MB ]; then
        script_exit "ANOTHER_NAME_FOR_SOFTWARE requires at least $MIN_MEM_MB MB of RAM to run. found $mem_mb MB." $ERR_INSUFFICIENT_REQUIREMENTS
    fi

    local disk_space_mb=$(df -m . | tail -1 | awk '{print $4}')
    if [ $disk_space_mb -lt $MIN_DISK_SPACE_MB ]; then
        script_exit "ANOTHER_NAME_FOR_SOFTWARE requires at least $MIN_DISK_SPACE_MB MB of free disk space for installation. found $disk_space_mb MB." $ERR_INSUFFICIENT_REQUIREMENTS
    fi
...
install_on_debian()
{
    ...
    if check_if_pkg_is_installed this_software_name; then
        pkg_version=$($ANOTHER_NAME_FOR_SOFTWARE_VERSION_CMD) || script_exit "unable to fetch the app version. please upgrade to latest version $?" $ERR_INTERNAL
        echo "[i] ANOTHER_NAME_FOR_SOFTWARE already installed ($pkg_version)"
        return
    fi
    ...
    rm -f any_company.list > /dev/null
    run_quietly "curl -s -o any_company.list $PMC_URL/$DISTRO/$SCALED_VERSION/$CHANNEL.list" "unable to fetch repo list" $ERR_FAILED_REPO_SETUP
    run_quietly "mv ./any_company.list /etc/apt/sources.list.d/any_company-$CHANNEL.list" "unable to copy repo to location" $ERR_FAILED_REPO_SETUP

    ### Fetch the gpg key ###
    run_quietly "curl -s https://packages.any_company.com/keys/any_company.asc | apt-key add -" "unable to fetch the gpg key" $ERR_FAILED_REPO_SETUP
    run_quietly "apt-get update" "[!] unable to refresh the repos properly"

    ### Install ANOTHER_NAME_FOR_SOFTWARE ###
    echo "[>] installing ANOTHER_NAME_FOR_SOFTWARE"
    if [ "$CHANNEL" = "prod" ]; then
        if [[ -z "$VERSION_NAME" ]]; then
            run_quietly "$PKG_MGR_INVOKER install this_software_name" "unable to install ANOTHER_NAME_FOR_SOFTWARE ($?)" $ERR_INSTALLATION_FAILED
        else
            run_quietly "$PKG_MGR_INVOKER -t $VERSION_NAME install this_software_name" "unable to install ANOTHER_NAME_FOR_SOFTWARE ($?)" $ERR_INSTALLATION_FAILED
        fi
    else
        run_quietly "$PKG_MGR_INVOKER -t $CHANNEL install this_software_name" "unable to install ANOTHER_NAME_FOR_SOFTWARE ($?)" $ERR_INSTALLATION_FAILED
    fi
...
install_on_fedora()
{
    ...
    if check_if_pkg_is_installed this_software_name; then
        pkg_version=$($ANOTHER_NAME_FOR_SOFTWARE_VERSION_CMD) || script_exit "Unable to fetch the app version. Please upgrade to latest version $?" $ERR_INSTALLATION_FAILED
        echo "[i] ANOTHER_NAME_FOR_SOFTWARE already installed ($pkg_version)"
        return
    fi

    repo=packages-any_company-com
    ...
    if [[ $SCALED_VERSION == 7* ]] && [[ "$CHANNEL" != "prod" ]]; then
        repo=packages-any_company-com-prod
    fi
    ...
    ### Fetch the gpg key ###
    run_quietly "curl https://packages.any_company.com/keys/any_company.asc > any_company.asc" "unable to fetch gpg key $?" $ERR_FAILED_REPO_SETUP
    run_quietly "rpm $(get_rpm_proxy_params) --import any_company.asc" "unable to import gpg key" $ERR_FAILED_REPO_SETUP
    run_quietly "yum makecache" " Unable to refresh the repos properly. Command exited with status ($?)"

    ### Install ANOTHER_NAME_FOR_SOFTWARE ###
    echo "[>] installing ANOTHER_NAME_FOR_SOFTWARE"
    run_quietly "$PKG_MGR_INVOKER --enablerepo=$repo-$CHANNEL install this_software_name" "unable to install ANOTHER_NAME_FOR_SOFTWARE ($?)" $ERR_INSTALLATION_FAILED
...
install_on_sles()
{
    ...
    lines=$($PKG_MGR_INVOKER lr | grep "packages-any_company-com-$CHANNEL" | wc -l)

    if [ $lines -eq 0 ]; then
        run_quietly "$PKG_MGR_INVOKER addrepo -c -f -n any_company-$CHANNEL https://packages.any_company.com/config/$DISTRO/$SCALED_VERSION/$CHANNEL.repo" "unable to load repo" $ERR_FAILED_REPO_SETUP
    fi

    ### Fetch the gpg key ###
    run_quietly "rpm $(get_rpm_proxy_params) --import https://packages.any_company.com/keys/any_company.asc > any_company.asc" "unable to fetch gpg key $?" $ERR_FAILED_REPO_SETUP
    ...
    run_quietly "$PKG_MGR_INVOKER install $ASSUMEYES $repo-$CHANNEL:this_software_name" "[!] failed to install ANOTHER_NAME_FOR_SOFTWARE (1/2)"
    
    if ! check_if_pkg_is_installed this_software_name; then
        echo "[r] retrying"
        sleep 2
        run_quietly "$PKG_MGR_INVOKER install $ASSUMEYES this_software_name" "unable to install ANOTHER_NAME_FOR_SOFTWARE 2/2 ($?)" $ERR_INSTALLATION_FAILED
    fi
...
remove_repo()
{
    ...
    if [ $DISTRO == 'sles' ] || [ "$DISTRO" = "sle-hpc" ]; then
        run_quietly "$PKG_MGR_INVOKER removerepo packages-any_company-com-$CHANNEL" "failed to remove repo"
    else
        script_exit "unsupported distro for remove repo $DISTRO" $ERR_UNSUPPORTED_DISTRO
    fi
}

upgrade_this_software_name()
{
    if [ -z "$1" ]; then
        script_exit "INTERNAL ERROR. upgrade_this_software_name requires an argument (the upgrade command)" $ERR_INTERNAL
    fi

    if ! check_if_pkg_is_installed this_software_name; then
        script_exit "ANOTHER_NAME_FOR_SOFTWARE package is not installed. Please install it first" $ERR_ANOTHER_NAME_FOR_SOFTWARE_NOT_INSTALLED
    fi

    run_quietly "$PKG_MGR_INVOKER $1 this_software_name" "Unable to upgrade ANOTHER_NAME_FOR_SOFTWARE $?" $ERR_INSTALLATION_FAILED
    echo "[v] upgraded"
}

remove_this_software_name()
{
    if ! check_if_pkg_is_installed this_software_name; then
        script_exit "ANOTHER_NAME_FOR_SOFTWARE package is not installed. Please install it first"
    fi

    run_quietly "$PKG_MGR_INVOKER remove this_software_name" "unable to remove ANOTHER_NAME_FOR_SOFTWARE $?" $ERR_UNINSTALLATION_FAILED
...
onboard_device()
{
    ...
    # validate onboarding
    sleep 3
    if [[ $(this_software_name --configured | grep "$this_software_name is not configured" -c) -gt 0 ]]; then
...
{
    # echo "[>] setting ANOTHER_NAME_FOR_SOFTWARE/EPP to passive mode"

    if ! check_if_pkg_is_installed this_software_name; then
        script_exit "ANOTHER_NAME_FOR_SOFTWARE package is not installed. Please install it first" $ERR_ANOTHER_NAME_FOR_SOFTWARE_NOT_INSTALLED
    fi

    retry_quietly 3 "this_software_name config passive-mode --value enabled" "failed to set ANOTHER_NAME_FOR_SOFTWARE to passive-mode" $ERR_PARAMETER_SET_FAILED
...
set_device_tags()
{
    ...
    do
        ...
        if [ "$1" == "GROUP" ] || [ "$1" == "SecurityWorkspaceId" ] || [ "$1" == "AzureResourceId" ] || [ "$1" == "SecurityAgentId" ]; then
            ...
            retry_quietly 2 "this_software_name edr tag set --name $1 --value $2" "failed to set tag" $ERR_PARAMETER_SET_FAILED
        else
...
usage()
{
    echo "ANOTHER_NAME_FOR_SOFTWARE_installer.sh v$SCRIPT_VERSION"
...
if [ "$INSTALL_MODE" == "i" ]; then
    ...
    if [ "$DISTRO_FAMILY" == "debian" ]; then
        upgrade_this_software_name "$ASSUMEYES install --only-upgrade"
    elif [ "$DISTRO_FAMILY" == "fedora" ]; then
        upgrade_this_software_name "$ASSUMEYES update"
    elif [ "$DISTRO_FAMILY" == "sles" ]; then
        upgrade_this_software_name "up $ASSUMEYES"
    else    
        script_exit "unsupported distro $DISTRO $VERSION" $ERR_UNSUPPORTED_DISTRO
    fi
...
script_exit "--- ANOTHER_NAME_FOR_SOFTWARE_installer.sh ended. ---" $SUCCESS